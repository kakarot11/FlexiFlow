...
import { insertUserSchema, insertContactSchema, insertWorkflowSchema, insertWorkflowStepSchema, insertAiAgentSchema, insertTaskSchema, insertActivitySchema, insertDomainTemplateSchema } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/workflows", async (req, res) => {
    try {
      ...
      const workflowData = insertWorkflowSchema.parse({ ...req.body, userId: user.id });
  ...
  app.post("/api/workflows/:id/steps", async (req, res) => {
    try {
      ...
      const stepData = insertWorkflowStepSchema.parse({ ...req.body, workflowId });
  ...
  app.post("/api/agents", async (req, res) => {
    try {
      ...
      const agentData = insertAiAgentSchema.parse({ ...req.body, userId: user.id });
  ...
  app.get("/api/templates", async (req, res) => {
    try {
      ...
      console.error(error);
      return res.status(500).json({ error: "Failed to fetch domain templates" });
    }
  });
  ...
  app.post("/api/ai/suggest-workflow", async (req, res) => {
    try {
      ...
      try {
        ...
        return res.status(500).json({ error: "Failed to generate workflow suggestions" });
      }
    } catch (error) {
      console.error(error);
      return res.status(500).json({ error: "An unexpected error occurred" });
  ...
  const createDemoData = async () => {
    try {
      ...
      if (existingWorkflows.length === 0) {
        const templates = await storage.getDomainTemplatesByDomain("real-estate");
          ...
          const tasks = [
            {
              ...
              status: "pending",
              workflowId: workflows.find(w => w.name === "Client Follow-up")?.id || workflows[0].id,
              contactId: contacts.find(c => c.name === "John Smith")?.id || contacts[0].id
            },
            {
              ...
              status: "in-progress",
              workflowId: workflows.find(w => w.name === "Closing Process")?.id || workflows[0].id,
              contactId: contacts.find(c => c.name === "Sarah Johnson")?.id || contacts[0].id
            },
            {
            ...
            },
            {
              ...
              status: "scheduled",
              workflowId: workflows.find(w => w.name === "Property Management")?.id || workflows[0].id,
              contactId: null
            }
          ];
        ...
        if (workflows.length > 0 && contacts.length > 0) {
          const activities = [
            ...
            },
            {
              ...
              contactId: contacts.find(c => c.name === "James Wilson")?.id || contacts[0].id,
